...
def subscription_plans(request):
    """
    View to display subscription plans and the user's
    ...
    current_subscription = (
        user_profile.subscription_plan if user_profile else None
    )
    is_paused = (
        user_profile.pause_approved if user_profile else False
    )
...
def subscribe(request, plan_id):
    ...
    if user_profile.subscription_plan:
        ...
        if current_plan.is_active:
            messages.error(
                request,
                f"You are already subscribed to the {current_plan.name} plan."
                f"Please cancel it before switching.",
            )
            ...
            messages.success(
                request,
                f"Successfully switched to {plan.name} plan!",
            )
    else:
        user_profile.subscription_plan = plan
        user_profile.save()
...
def cancel_subscription(request):
    ...
    """
    subscription_plan = (
        request.user.userprofile.subscription_plan
    )
    ...
    if subscription_plan and subscription_plan.is_active:
        ...
        messages.success(
            request, "Your subscription has been canceled."
        )
    ...
    try:
        ...
        if "plans" not in data:
            return JsonResponse(
                {
                    "status": "error",
                    "message": "No plans data provided",
                },
                status=400,
            )
...
def request_pause_subscription(request):
    ...
    if (
        not user_profile.pause_requested
        and request.user.userprofile.subscription_plan.is_active
    ):
        ...
        send_mail(
            ...
            f"{request.user.username} requested to pause their subscription.",
        ...
        messages.success(
            request,
            "Your subscription pause request is submitted, awaiting approval.",
        )
        return redirect("profile")
    else:
        messages.error(
            request,
            "You cannot request to pause your subscription at this time.",
        )
        return redirect("profile")
...
def resume_subscription(request):
    ...
    if user_profile.pause_approved:
        ...
        user_profile.save()
        messages.success(
            request, "Your subscription has been resumed."
        )
    else:
        messages.error(
            request, "Your subscription was not paused."
        )
    return redirect("profile")
...
def home(request):
    ...
    spotlight_products = Product.objects.filter(
        is_spotlight=True
    )[:6]
    community_updates = CommunityUpdate.objects.order_by(
        "-created_at"
    )[:5]
    spotlight_subscriptions = SubscriptionPlan.objects.filter(
        is_spotlight=True
    )[:3]
...
def remove_from_wishlist(request, product_id):
    ...
    try:
        ...
        messages.success(
            request, "Item removed from your wishlist."
        )
    except WishlistItem.DoesNotExist:
        messages.error(
            request, "Item not found in your wishlist."
        )
    return redirect("wishlist_view")
...
def post_update(request):
    ...
    if request.method == "POST":
        ...
        CommunityUpdate.objects.create(
            user=request.user, update_text=update_text
        )
...
def community_updates(request):
    """
    Display a list of community updates and handle newsletter
    ...
    if request.method == "POST":
        ...
        if action == "subscribe":
            ...
            try:
                ...
                messages.error(
                    request, "This email is not subscribed."
                )
        return redirect("community_updates")
    else:
    ...
    return render(
        request,
        "fitness/community_updates.html",
...
def update_profile(request):
    ...
    return render(
        request, "fitness/update_profile.html", {"form": form}
    )
...
def create_review(request, product_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Your review was submitted and pending approval by an admin.",
            )
            return redirect(
...
def edit_review(request, review_id):
    ...
    if request.method == "POST":
        ...
        review.save()
        return redirect(
            "product_detail", product_id=review.product.id
        )
...
def contact_form(request):
    ...
    return render(
        request, "fitness/contact_form.html", {"form": form}
    )
